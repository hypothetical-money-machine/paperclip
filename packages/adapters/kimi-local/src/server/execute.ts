...
// Kimi CLI discovers registered skills via the `.kimi/skills/` directory
// structure when `--skills-dir` is passed; symlink each skill into that layout.
  ...
  for (const candidate of PAPERCLIP_SKILLS_CANDIDATES) {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        await fs.symlink(path.join(candidate, entry.name), path.join(target, entry.name));
      }
    }
    break;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // Including the path directive lets the model resolve relative file references;
  // write to tmp so we don't modify the source file.
